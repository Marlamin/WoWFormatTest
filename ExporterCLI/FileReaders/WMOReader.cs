    ...
    public class WMOReader
    {
        ...
        private MODD[] ReadMODDChunk(uint size, BinaryReader bin)
        {
            ...
            for (var i = 0; i < numDoodads; i++)
            {
                ...
                doodads[i].position = bin.Read<Utils.Vector3>();
                doodads[i].rotation = bin.Read<Utils.Quaternion>();
        ...
        private MOGP ReadMOGPChunk(uint size, BinaryReader bin)
        {
            var mogp = new MOGP()
            {
                ...
                boundingBox1 = bin.Read<Utils.Vector3>(),
                boundingBox2 = bin.Read<Utils.Vector3>(),
        ...
        private MONR[] ReadMONRChunk(uint size, BinaryReader bin)
        {
            ...
            {
                normals[i].normal = bin.Read<Utils.Vector3>();
            }
            return normals;
        }
        private MOVT[] ReadMOVTChunk(uint size, BinaryReader bin)
        {
            ...
            {
                vertices[i].vector = bin.Read<Utils.Vector3>();
            }
            return vertices;
        }
