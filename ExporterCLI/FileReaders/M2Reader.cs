...
namespace WoWFormatLib.FileReaders
{
    public class M2Reader
    {
        ...
        public void ParseYeOldeM2Struct(Stream m2stream)
        {
            ...
            model.vertexbox = new Utils.Vector3[2];
            model.vertexbox[0] = bin.Read<Utils.Vector3>();
            model.vertexbox[1] = bin.Read<Utils.Vector3>();
            model.vertexradius = bin.ReadSingle();
            model.boundingbox = new Utils.Vector3[2];
            model.boundingbox[0] = bin.Read<Utils.Vector3>();
            model.boundingbox[1] = bin.Read<Utils.Vector3>();
        ...
        private Texture[] ReadTextures(uint num, uint offset, BinaryReader bin)
        {
